...
<!--    </div>-->
    <el-table
      ...
      >
        <template slot-scope="scope">
          <el-button v-if="scope.row.principalReview == 0" @click="schoolSysCheck(scope.row,1)" size="small" type="primary">通过</el-button>
          <el-button v-if="scope.row.principalReview == 0" @click="schoolSysCheck(scope.row,2)" size="small" type="danger">拒绝</el-button>
          <el-button v-if="scope.row.principalReview == 1"  size="small" type="primary">已通过</el-button>
          <el-button v-if="scope.row.principalReview == 2"  size="small" type="danger">已拒绝</el-button>
      ...
      schoolSysCheck(data,type){
        if(type == 2){
          this.$prompt('请输入拒绝通过原因', '提示', {
            ...
            await this.schoolSysCheckFn(data.id,type,value)
          }).catch(() => {
          ...
          this.$confirm('确定审核通过此换班申请吗?', '提示', {
            ...
            await this.schoolSysCheckFn(data.id,type)
          }).catch(() => {
          });
        }
      },
      schoolSysCheckFn(id,type,mark){
        var obj = {
          principalReview:type,
          id:id
        };
        if(type == 2){ //拒绝
          obj.principalRemarks = mark;
        }
        this.http.post('/classChange/updateSchoolClassChangeByPrincipa',obj).then(res=>{
          if(res.code == 0){
