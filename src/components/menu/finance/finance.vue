    ...
    <el-tabs v-model="activeName" >
      <el-tab-pane label="待审核列表" name="first">
        <div style="display:flex;justify-content: center">
          ...
          <el-select filterable style="margin-right: 10px;" v-model="getobj.isVerify" placeholder="请选择状态">
            <el-option
              :key="11"
              label="无"
              :value="null">
  ...
  export default {
    ...
    methods:{
      ...
      getWaitList(){
        ...
        this.http.post('/money/queryMoneyVerifyList',this.getobj).then(res=>{
          if(res.code == 0){
      ...
      getAlreadyList(){
        this.http.post('/money/queryMoneyVerifyList',{isVerify:1,pageNum:this.apagenum,pageSize:10 }).then(res=>{
          if(res.code == 0){
      ...
      getCloseList(){
        this.http.post('/money/queryMoneyVerifyList',{isVerify:2,pageNum:this.cpagenum,pageSize:10 }).then(res=>{
          if(res.code == 0){
      ...
      againOpen(data){
        this.$prompt('请输入原因', '提示', {
          ...
          await this.shenheOrderFn(data.orderList.orderId,data.studentId,1,value);
        }).catch(() => {
      ...
      //订单操作
      shenheOrderFn(oid,sid,isv,v){ //isv 1通过 2和3是不通过或挂壁
        this.http.post('/money/updateStudentOrder',{orderId:oid,studentId:sid,isVerify:isv,detail:v}).then(res=>{
          if(res.code == 0){
