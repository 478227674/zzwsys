...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      getAlreadyList(){
        this.http.post('/money/queryMoneyNoDirectVerifyList',{schoolId:this.schoolId,isVerify:1,pageNum:this.apagenum,pageSize:10 }).then(res=>{
          if(res.code == 0){
      ...
      getCloseList(){
        this.http.post('/money/queryMoneyNoDirectVerifyList',{schoolId:this.schoolId,isVerify:2,pageNum:this.cpagenum,pageSize:10 }).then(res=>{
          if(res.code == 0){
      ...
      //订单操作
      shenheOrderFn(oid,sid,isv,value){ //isv 1通过 2和3是不通过或挂壁
