  ...
  <div style="padding: 20px;">
    ...
    <el-dialog title="添加业务员" :visible.sync="addUsreFlag">
      <el-form :model="saleobj">
        ...
        <el-form-item label="选择分校" :label-width="formLabelWidth">
          <el-select filterable v-model="saleobj.schoolId" placeholder="请选择分校" >
            <el-option
        ...
        <el-form-item label="选择管理员" :label-width="formLabelWidth">
          <el-select filterable v-model="saleobj.schoolUserId" placeholder="请选择管理员" >
            <el-option
    ...
    <el-dialog title="编辑业务员" :visible.sync="editUsreFlag">
      <el-form :model="editsaler">
        ...
        <el-form-item label="选择分校" :label-width="formLabelWidth">
          <el-select filterable v-model="editsaler.schoolId" placeholder="请选择分校" >
            <el-option
              v-for="(item,index) in schoolList"
              :key="index"
              :label="item.schoolName"
        ...
        <el-form-item label="选择管理员" :label-width="formLabelWidth">
          <el-select filterable v-model="editsaler.schoolUserId" placeholder="请选择管理员" >
            <el-option
              v-for="(item,index) in sysList"
              :key="index"
              :label="item.userName"
  ...
  export default {
    ...
    methods:{
      ...
      updateSys(data){
        ...
        this.editsaler.schoolUserId = data.schoolUserId;
        this.editsaler.schoolId = data.schoolId;
      },
      //添加业务员
      submitAddSaler() {
        ...
        if(!this.saleobj.schoolId){
          this.$errorMessage('请选择分校')
          return;
        }
        if(!this.saleobj.schoolUserId){
          this.$errorMessage('请选择管理员')
          return;
        }
      ...
      submitEditsaler(){
        ...
        if(this.editsaler.isMaster==1){
          if(!this.editsaler.schoolId){
            this.$errorMessage('请选择分校')
            return;
          }
        }
        if(!this.editsaler.schoolUserId){
          this.$errorMessage('请选择管理员')
          return;
        }
      ...
      deleteSaler(data){
        this.$confirm('确定删除此业务员吗?', '提示', {
          ...
          await this.deleteSalerFn(data.saleId)
        }).catch(() => {
          this.$message({
            type: 'info',
            message: '已取消'
      ...
      deleteSalerFn(id){
        this.http.post('/school/deleteSchoolSale',{saleId:id}).then(res=>{
          if(res.code == 0){
